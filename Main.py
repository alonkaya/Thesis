...
if __name__ == "__main__":
        ...
        for i, (alg_coeff, re1_coeff, sed_coeff, data_ratio, lr, bs) in enumerate(param_combinations):
                ...
                num_epochs = 2000 if data_ratio==0.3 else 3000 if data_ratio==0.2 else 4000 if data_ratio==0.1 else 5500 if data_ratio==0.05 else 6000 if data_ratio==0.025 else 0
...
use_reconstruction_{USE_RECONSTRUCTION_LAYER}__BS_{bs}{dataset_class}__ratio_{data_ratio}__head_{HEAD}__frozen_{FROZEN_LAYERS}""")
                ...
                if model.start_epoch < model.num_epochs:
                        ...
                        SVD coeff: {LAST_SV_COEFF}, RE1 coeff: {re1_coeff} SED coeff: {sed_coeff}, ALG_COEFF: {alg_coeff}, L2_coeff: {L2_coeff}, huber_coeff: {huber_coeff}, frozen layers: {FROZEN_LAYERS}, group conv: {GROUP_CONV["use"]}

...
if __name__ == "__main__":
    ...
    param_combinations = itertools.product(batch_sizes, penalty_coeffs, penaltize_normalized_options, learning_rates_vit, learning_rates_mlp)
    
    for batch_size, penalty_coeff, penaltize_normalized, lr_vit, lr_mlp in param_combinations:
        model = FMatrixRegressor(MLP_HIDDEN_DIM, NUM_OUTPUT, 
                                pretrained_model_name=CLIP_MODEL_NAME, lr_vit=lr_vit, lr_mlp=lr_mlp,
                                penalty_coeff=penalty_coeff, batch_size=batch_size, batchnorm_and_dropout=BATCHNORM_AND_DROPOUT,
...
batch_size: {batch_size}, train_seqeunces: {train_seqeunces}, val_sequences: {val_sequences}, penaltize_normalized: {penaltize_normalized}, RealEstate: {USE_REALESTATE} batchnorm & dropout: {BATCHNORM_AND_DROPOUT}\n\n"""

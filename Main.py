...
if __name__ == "__main__":
        ...
        for i, (alg_coeff, re1_coeff, sed_coeff, seed, train_size, lr, part, fl) in enumerate(param_combinations):
                ...
                if SCENEFLOW and FLYING:
                        num_epochs = 8000 if train_size==170 else 18000 if train_size==80 else 35000 if train_size==22 else 45000 if train_size==9 else 0
                else:
                        num_epochs = 2000 if train_size==0.3 else 4500 if train_size==0.2 else 7000 if train_size==0.1 else \
                                     ...
                                     40000 if train_size==0.015 else 65000 if train_size==0.008 else 100000 if train_size==0.004 else 150000 if train_size==0.002 else 0
                if num_epochs == 0:
                ...
                model = FMatrixRegressor(lr=lr, min_lr=MIN_LR, batch_size=batch_size, L2_coeff=L2_coeff, huber_coeff=huber_coeff, alg_coeff=alg_coeff, re1_coeff=re1_coeff, sed_coeff=sed_coeff, \
                                         plots_path=plots_path, trained_vit=TRAINED_VIT, pretrained_path=PRETRAINED_PATH, num_epochs=num_epochs, frozen_layers=fl).to(device)
...
RE1 coeff: {re1_coeff} SED coeff: {sed_coeff}, ALG_COEFF: {alg_coeff}, L2_coeff: {L2_coeff}, huber_coeff: {huber_coeff}, frozen layers: {fl}, trained vit: {TRAINED_VIT}, CC: {CC},

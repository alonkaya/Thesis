...
def plot_errors():
    ...
    frac = 0.95
    # KITTI Frozen: 0
    mean_alg_0 = [x*frac for x in [0.22, 0.3033, 0.25867, 0.3233, 0.36, 0.3933, 0.4667, 0.6267]]
    mean_SED_0 = [x*frac for x in [0.199666667, 0.353333, 0.272333, 0.38, 0.45, 0.506667, 0.67, 1.06]]
    mean_RE1_0 = [x*frac for x in [0.055, 0.106667, 0.073333, 0.116667, 0.146667, 0.173333, 0.226667, 0.486667]]
    std_alg_0 = [x*frac for x in [0.011547005, 0.025166, 0.025007, 0.015275, 0.078102, 0.070946, 0.025166, 0.141892]]
    std_SED_0 = [x*frac for x in [0.011547, 0.058595, 0.037233, 0.01, 0.130767, 0.138684, 0.06, 0.288444]]
    std_RE1_0 = [x*frac for x in [0, 0.028868, 0.011547, 0.005774, 0.049329, 0.077675, 0.030551, 0.225906]]

    # KITTI Frozen: 4
    mean_alg_4 = [x*frac for x in [0.216667, 0.303333, 0.248667, 0.306667, 0.366667, 0.373333, 0.446667, 0.57]]
    mean_SED_4 = [x*frac for x in [0.195667, 0.326667, 0.253333, 0.36, 0.456667, 0.476667, 0.626667, 0.92]]
    mean_RE1_4 = [x*frac for x in [0.05, 0.096, 0.066667, 0.113333, 0.143333, 0.163333, 0.22, 0.4]]
    std_alg_4 = [x*frac for x in [0.011547, 0.047258, 0.024194, 0.015275, 0.037859, 0.066583, 0.061101, 0.079373]]
    std_SED_4 = [x*frac for x in [0.005859, 0.068069, 0.025166, 0.03, 0.080829, 0.124231, 0.135031, 0.240624]]
    std_RE1_4 = [x*frac for x in [8.5e-18, 0.0151, 0.011547, 0.011547, 0.023094, 0.066583, 0.06, 0.160935]]

    # KITTI Frozen: 8
    mean_alg_8 = [x*frac for x in [0.213333, 0.283333, 0.26, 0.326667, 0.376667, 0.346667, 0.503333, 0.633333]]
    mean_SED_8 = [x*frac for x in [0.199, 0.31, 0.293333, 0.386667, 0.456667, 0.44, 0.763333, 1.053333]]
    mean_RE1_8 = [x*frac for x in [0.047, 0.091667, 0.083333, 0.123333, 0.153333, 0.14, 0.28, 0.483333]]
    std_alg_8 = [x*frac for x in [0.025166115, 0.02081666, 0.03, 0.023094011, 0.015275252, 0.06350853, 0.056862407, 0.096090235]]
    std_SED_8 = [x*frac for x in [0.037322915, 0.036055513, 0.055075705, 0.025166115, 0.032145503, 0.121243557, 0.167431578, 0.243378991]]
    std_RE1_8 = [x*frac for x in [0.011269428, 0.017559423, 0.02081666,  0.005773503, 0.02081666, 0.060827625, 0.075498344, 0.191398363]]

    # KITTI Frozen: top 5
    mean_alg_5 = [x*frac for x in [0.226666667, 0.315333333,	0.335333333,	0.331,	0.404666667,	0.452666667,	0.47,	0.686]]
    mean_SED_5 = [x*frac for x in [0.21,	0.356666667, 0.38,	0.402,	0.52,	0.64,	0.676,	1.15666666]]
    mean_RE1_5 = [x*frac for x in [0.054666667,	0.106666667, 0.115,	0.128333333,	0.176666667,	0.223333333,	0.257666667,	0.53]]
    std_alg_5 = [x*frac for x in [0.005773503,	0.02532456,	0.009237604,	0.025357445,	0.054197171,	0.071143048,	0.052915026,	0.084071398]]
    std_SED_5 = [x*frac for x in [0,	0.045092498, 0.038935845,	0.031176915,	0.085440037,	0.165227116,	0.105910339,	0.160104133]]
    std_RE1_5 = [x*frac for x in [0.004163332,	0.02081666,	0.018027756, 0.01106044,	0.035118846,	0.077674535,	0.058620247,	0.051961524]]

    # KITTI Resnet #
    resnet_mean_alg_0 = [x*frac for x in [0.246666667, 0.37, 0.378666667, 0.353333333, 0.423333333, 0.466666667, 0.42, 0.508333333]]
    resnet_std_alg_0 = [x*frac for x in [0.135769412, 0.05, 0.043650124, 0.065064071, 0.072341781, 	0.066583281, 0.173205081, 0.007637626]]
    resnet_mean_SED_0 = [x*frac for x in [0.476666667, 0.463333333, 0.48, 0.413333333, 0.55, 0.586666667,  0.576666667, 0.696666667]]
    resnet_std_SED_0 = [x*frac for x in [0.255799401, 0.090737717, 0.081853528, 0.105039675, 0.132287566, 0.092915732, 0.351046056, 0.032145503]]
    resnet_mean_RE1_0 = [x*frac for x in [0.083333333, 0.196666667, 0.166666667,	 0.146666667, 0.206666667,	 0.243333333, 0.21,	 0.291666667]]
    resnet_std_RE1_0 = [x*frac for x in [0.082512625, 0.077674535, 0.030550505, 0.055075705, 0.061101009, 0.046188022, 0.147986486, 0.032532035]]

    # KITTI: Clip 16
    clip_16_mean_alg = [x*frac for x in [0.206666667, 0.245, 0.26, 0.295, 0.318, 0.266333333, 0.400333333, 0.46]]
    clip_16_mean_SED = [x*frac for x in [0.176666667, 0.233333333, 0.283333333, 0.33, 0.366, 0.346666667, 0.531, 0.653333333]]
    clip_16_mean_RE1 = [x*frac for x in [0.046666667, 0.064666667, 0.082666667, 0.1, 0.11, 0.083, 0.173333333, 0.24666666]]
    clip_16_std_alg = [x*frac for x in [0.04163332, 0.035355339, 0.026457513, 0.06, 0.051, 0.110545617, 0.025735838, 0.105830052]]
    clip_16_std_SED = [x*frac for x in [0.049328829, 0.05033223, 0.061101009, 0.11, 0.1, 0.092915732, 0.059253692, 0.192180471]] 
    clip_16_std_RE1 = [x*frac for x in [0.015275252, 0.023180452, 0.02532456, 0.035, 0.036, 0.065551506, 0.025166115, 0.119303534]]

    # Flying
    flying_clip_alg = [x*frac for x in [0.36, 0.39, 1]]
    flying_clip_SED = [x*frac for x in [0.71, 0.8, 3.34]]
    flying_clip_RE1 = [x*frac for x in [0.14, 0.16, 1.19]]
    flying_clip_16_alg = [x*frac for x in [0.38, 0.34, 0.738]]
    flying_clip_16_SED = [x*frac for x in [0.73, 0.71, 2.34]]
    flying_clip_16_RE1 = [x*frac for x in [0.14, 0.138, 0.689]]
    flying_resnet_alg = [x*frac for x in [0.41, 0.44, 0.78]]
    flying_resnet_SED = [x*frac for x in [0.84, 1, 2.57]]
    flying_resnet_RE1 = [x*frac for x in [0.175, 0.21, 0.73]]

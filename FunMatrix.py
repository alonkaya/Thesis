...
if __name__ == "__main__":
    ...
    k_stereo_seq_09 = torch.tensor([[707.0912,   0.0000, 601.8873],
                                    ...
                                    [  0.0000,   0.0000,   1.0000]])
    E_gt_stereo = compute_essential(R, t_stereo)
    ...
    # k_est= torch.tensor([[147.6471,   0.0000,  99.6796],
    ...
    # F_est = torch.tensor([[[ 3.1915e-06,  2.3123e-03, -1.9637e-01],
    #         [-2.2769e-03,  9.8166e-05, -6.8320e-01],
    #         [ 1.9409e-01,  6.7341e-01,  5.9211e-02]]])
    # p1 = np.array([109.4683,  33.1415,   1.0000])
    # p2 = np.array([107.8517,  33.0934,   1.0000])
    ...
    # p1 = np.array([58.3084,  45.5892,   1.0000])
    # p2 = np.array([59.0644,  45.5891,   1.0000])


    E_est, R_est, T_est = ComputeEssentialMatrix(F_est.numpy(), k_stereo_seq_09.numpy(), k_stereo_seq_09.numpy(), p1[:2], p2[:2])
    q_est = QuaternionFromMatrix(R_est)

    E_gt, R_gt, T_gt = ComputeEssentialMatrix(F_gt_seq_9.numpy(), k_stereo_seq_09.numpy(), k_stereo_seq_09.numpy(), p1[:2], p2[:2])

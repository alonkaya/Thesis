...
class Inception(nn.Module):
    def __init__(self, in_channels, conv11_size, conv33_reduce_size, conv33_size,
        ...
        self.pool_proj = nn.Conv2d(in_channels, pool11_size, kernel_size=1)
    ...
    def forward(self, x):
        ...
        pool_proj = F.max_pool2d(x, kernel_size=3, stride=1, padding=1)
        pool11 = F.relu(self.pool_proj(pool_proj))
        
        return torch.cat([conv11, conv33, conv55, pool11], 1)
...
class FullyConvNetwork(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv33_reduce = nn.Conv2d(480, 96, kernel_size=1)
        self.conv33_reduce_param = nn.Conv2d(96, 32, kernel_size=1)
        self.conv33_reduce_param2 = nn.Conv2d(32, 32, kernel_size=1)
        self.conv33_reduce_param3 = nn.Conv2d(32, 6, kernel_size=1)
        self.conv55_reduce = nn.Conv2d(480, 16, kernel_size=1)
        self.conv55_reduce_param = nn.Conv2d(16, 32, kernel_size=1)
        self.conv55_reduce_param2 = nn.Conv2d(32, 32, kernel_size=1)
        self.conv55_reduce_param3 = nn.Conv2d(32, 6, kernel_size=1)
    ...
    def forward(self, x):
        ...
        pool11 = F.relu(self.conv11(pool_proj))
...
class UniversalCorrespondenceNetwork(nn.Module):
    def __init__(self, in_channels, img_shape):
        ...
        self.network1 = FullyConvNetwork(in_channels)
        self.network2 = FullyConvNetwork(in_channels)

    def forward(self, x1, x2):
        feature1 = self.network1(x1)
        feature2 = self.network2(x2)
        return feature1, feature2
    
class FeatureExtractorDeepF(nn.Module):
    def __init__(self, in_channels, img_shape):
        super(FeatureExtractorDeepF, self).__init__()
        self.ucn = UniversalCorrespondenceNetwork(in_channels, img_shape)
    ...
    def forward(self, x1, x2):
        # UCN
        feature1, feature2 = self.ucn.forward(x1, x2)

        # Concat
        concatenated_features = torch.cat([feature1, feature2], dim=1)

        # Convolutional layers after concatenation
        conv3x3 = F.relu(self.bn_3x3(self.conv_3x3(concatenated_features)))
        conv1x1 = F.relu(self.bn_1x1(self.conv_1x1(conv3x3)))

        # Pooling
        pooled_features, indices = self.pool(conv1x1)
        print(pooled_features.shape, indices.shape)
        
        # Use the indices to preserve spatial information
        indices = indices.float() / (pooled_features.shape[2] * pooled_features.shape[3])
        indices = indices.expand_as(pooled_features)
        
        pooled_features_with_position = torch.cat((pooled_features, indices), dim=1)
